  ...
  const fetchPastOrders = async () => {
    try {
      const [deliveredRes, cancelledRes] = await Promise.all([
        axios.get(`http://localhost:9090/api/v1/orders/couriers/${courierId}?status=DELIVERED`),
        axios.get(`http://localhost:9090/api/v1/orders/couriers/${courierId}?status=CANCELLED`)
      ]);

      const response = [...deliveredRes.data, ...cancelledRes.data];
      ...
      setOrders(mapped as ActiveOrder[]);

    } catch (err) {
      console.error("Active del error", err);
    }
  };
  ...
  return (
    <div className="p-4">
          ...
          {orders.length === 0 ? (
            ...
            orders.map((order) => (
              <tr key={order.orderId}>
                ...
                <td className="border p-2">{order.courierRate}</td>
      ...
      <div className="md:hidden space-y-4">
        {orders.length === 0 ? (
          <p className="text-center">No past deliveries.</p>
        ) : (
          orders.map((order) => (
            <div
              key={order.orderId}
